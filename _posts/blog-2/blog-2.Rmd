...
### Overall case distribution in Germany




```{r overall-plot,fig.height=6,fig.width=7,fig.cap="Overall case distribution in Germany" }
germany %>% 
  mutate(month=month(date))%>%
  group_by(type,month) %>%
  summarise(total_cases = sum(cases))%>%
  rename(Type=type)%>%
ggplot(aes(x=month,y=total_cases,col=Type))+ 
  geom_line()+ 
  geom_point()+
  labs(x = "", y = "Daily count", title =  "Case distribution - Monthly", subtitle = "Germany")+
 theme_gray()+
  theme(legend.position="bottom",
        legend.direction="horizontal",
        legend.box.background = element_rect(fill = "transparent"),
        legend.background = element_rect(fill = "transparent",linetype="solid",color = "#9bc1e0"),
        axis.text = element_text(size = 10),
        legend.text = element_text(size = 10), 
        legend.title = element_text(size=14),
        axis.title = element_text(size = 14), 
        axis.line = element_line(size = 0.4, colour = "white"),
        plot.background = element_rect(fill = "#9bc1e0"),
